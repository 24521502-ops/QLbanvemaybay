...
public class AircraftGUI extends JPanel {
    ...
    private void initComponents() {
        ...
        btnAdd.setPreferredSize(new Dimension(175, 42));
    ...
    private void showEditDialog(int row) {
        if (row < 0 || row >= allData.size())
            return;
        showCustomDialog("Cập nhật loại máy bay", "Chỉnh sửa thông tin loại máy bay đã chọn.", allData.get(row), row);
    }
    ...
    private void deleteRow(int row) {
        if (row < 0 || row >= allData.size())
            return;
        AircraftDTO dto = allData.get(row);
        ...
        JLabel lblIcon = new JLabel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                if (iconType == 1) {
                    ...
                    g2.drawLine(24, 24, 24, 18);
                    g2.drawLine(24, 24, 29, 24);
                } else {
                    g2.drawOval(14, 14, 20, 20);
