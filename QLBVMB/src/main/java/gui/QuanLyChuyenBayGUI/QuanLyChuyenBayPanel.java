...
public class QuanLyChuyenBayPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        btnAdd.addActionListener(e -> new AddFlightDialog((Frame) SwingUtilities.getWindowAncestor(this), true, this).setVisible(true));
        ...
        btnEditInfo.addActionListener(e -> {
            ...
            if(row == -1) { JOptionPane.showMessageDialog(this, "Vui lòng chọn 1 chuyến bay!"); return; }
        ...
        btnCancel.addActionListener(e -> {
            ...
            if(row == -1) { JOptionPane.showMessageDialog(this, "Vui lòng chọn 1 chuyến bay!"); return; }
            String id = table.getValueAt(row, 7).toString();
            if (JOptionPane.showConfirmDialog(this, "Hủy chuyến bay này?", "Xác nhận", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                if(chuyenBayBUS.huyChuyenBay(id)) { loadDataToTable(); }
            }
        });
    ...
    private JButton makePrimaryButton(String text) {
        JButton btn = new JButton(text) {
            @Override protected void paintComponent(Graphics g) {
                ...
                GradientPaint gp = new GradientPaint(0, 0, AppColor.PRIMARY, getWidth(), getHeight(), AppColor.PRIMARY_ACTIVE);
                g2.setPaint(gp);
    ...
    private JButton makeDestructiveButton(String text) {
        JButton btn = new JButton(text) {
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                GradientPaint gp = new GradientPaint(0, 0, AppColor.ERROR, getWidth(), getHeight(), AppColor.ERROR_ACTIVE);
                g2.setPaint(gp);
                g2.fillRoundRect(0, 0, getWidth(), getHeight(), 10, 10);
                g2.dispose();
                super.paintComponent(g);
    ...
    private void customizeTable() {
        ...
        DefaultTableCellRenderer cellRenderer = new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable t, Object val, boolean sel, boolean foc, int r, int c) {
                ...
                setBorder(BorderFactory.createEmptyBorder(0, 15, 0, 15));
                if (!sel) setBackground(r % 2 == 0 ? AppColor.SURFACE : new Color(252, 252, 253));
                
                if (c == 6 && val != null) {
                    setFont(new Font("Inter", Font.BOLD, 13));
                    String status = val.toString().toUpperCase();
                    if (status.contains("CANCEL")) setForeground(AppColor.ERROR);
                    else if (status.contains("DELAY")) setForeground(AppColor.WARNING);
                    else setForeground(AppColor.SUCCESS);
                } else {
        ...
        for (int i = 0; i < table.getColumnModel().getColumnCount() - 1; i++) table.getColumnModel().getColumn(i).setCellRenderer(cellRenderer);
    }
    ...
    public void loadDataToTable() {
        ...
        for (Object[] row : flights) {
            if(row[2] != null) row[2] = sdf.format(row[2]);
            if(row[3] != null) row[3] = sdf.format(row[3]);
            tableModel.addRow(row);
            total++;
            String status = row[6].toString().toUpperCase(); 
            if (status.contains("SCHEDULED")) scheduled++;
            if (status.contains("DELAY")) delayed++;
        }
    ...
    private void showDelayDialog(String flightID, String flightNum, String oldTimeStr) {
        ...
        dialog.setSize(450, 320); dialog.setLocationRelativeTo(this); dialog.setLayout(new BorderLayout());
        ...
        body.add(lblInfo); body.add(Box.createVerticalStrut(20));
        ...
        body.add(lblInput); body.add(Box.createVerticalStrut(8));
        ...
        btnSave.addActionListener(e -> {
            try {
                ...
                if(chuyenBayBUS.delayChuyenBay(flightID, newTime)) { loadDataToTable(); dialog.dispose(); }
            } catch (Exception ex) { JOptionPane.showMessageDialog(dialog, "Sai định dạng!"); }
        });
        footer.add(btnSave); dialog.add(footer, BorderLayout.SOUTH); dialog.setVisible(true);
    }
