...
public class UpdateFlightDialog extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        header.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createMatteBorder(0, 0, 1, 0, OUTLINE_VARIANT), new EmptyBorder(20, 24, 20, 24)));
        ...
        txtSoHieu = makeInputField();
        body.add(makeFieldBlock("SỐ HIỆU CHUYẾN BAY", txtSoHieu));
        body.add(Box.createVerticalStrut(12));
        cbHangBay = makeComboBox();
        body.add(makeFieldBlock("HÃNG HÀNG KHÔNG", cbHangBay));
        body.add(Box.createVerticalStrut(12));
        cbTauBay = makeComboBox();
        body.add(makeFieldBlock("TÀU BAY THỰC HIỆN", cbTauBay));
        body.add(Box.createVerticalStrut(12));
        cbSanBayDi = makeComboBox();
        body.add(makeFieldBlock("SÂN BAY ĐI", cbSanBayDi));
        body.add(Box.createVerticalStrut(12));
        cbSanBayDen = makeComboBox();
        body.add(makeFieldBlock("SÂN BAY ĐẾN", cbSanBayDen));
        body.add(Box.createVerticalStrut(12));

        // Dùng Spinner
        spnKhoiHanh = makeDateSpinner();
        body.add(makeFieldBlock("THỜI GIAN KHỞI HÀNH", spnKhoiHanh));
        body.add(Box.createVerticalStrut(12));
        spnHaCanh = makeDateSpinner();
        body.add(makeFieldBlock("THỜI GIAN HẠ CÁNH", spnHaCanh));
        body.add(Box.createVerticalStrut(12));
    ...
    private void loadDropdownData() {
        try (Connection conn = DBConnection.getConnection(); Statement stmt = conn.createStatement()) {
            ...
            while (rsAL.next())
                cbHangBay.addItem(rsAL.getString(1) + " - " + rsAL.getString(2));

            ResultSet rsAC = stmt.executeQuery("SELECT AircraftID, Model FROM AIRCRAFT");
            while (rsAC.next())
                cbTauBay.addItem(rsAC.getString(1) + " - " + rsAC.getString(2));

            ResultSet rsAP = stmt.executeQuery("SELECT AirportID, AirportName FROM AIRPORT");
            while (rsAP.next()) {
                String ap = rsAP.getString(1) + " - " + rsAP.getString(2);
    ...
    private void loadOldData() {
        ...
        if (oldData != null) {
            ...
            if (oldData[5] != null)
                spnKhoiHanh.setValue((java.util.Date) oldData[5]);
            if (oldData[6] != null)
                spnHaCanh.setValue((java.util.Date) oldData[6]);
    ...
    private void btnSaveAction(java.awt.event.ActionEvent evt) {
        try {
            ...
            if (depAp.equals(arrAp)) {
                JOptionPane.showMessageDialog(this, "Sân bay đi và đến trùng nhau!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            JOptionPane.showMessageDialog(this, "Nhập sai ngày giờ!", "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private JTextField makeInputField() {
        ...
        tf.setFont(new Font("Inter", Font.PLAIN, 14));
        tf.setBackground(SURFACE_LOW);
        tf.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(OUTLINE_VARIANT),
                new EmptyBorder(8, 12, 8, 12)));
        return tf;
    }
