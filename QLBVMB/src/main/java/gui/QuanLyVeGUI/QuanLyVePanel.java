...
public class QuanLyVePanel extends JPanel {
    ...
    public QuanLyVePanel() {
        ...
        // Tự động tải lại dữ liệu mỗi khi người dùng click sang Tab Vé
    ...
    private void initComponents() {
        ...
        JMenuItem itemSortDateAsc = createMenuItem("Ngày bay (Tăng dần)");
        JMenuItem itemSortDateDesc = createMenuItem("Ngày bay (Giảm dần)");
        ...
        itemPriceAsc.addActionListener(e -> applySortingCustom(5));
        itemPriceDesc.addActionListener(e -> applySortingCustom(6));
        itemStatus.addActionListener(e -> applySortingCustom(7));
    ...
    private void applySortingCustom(int type) {
        ...
        // Tạo Comparator xử lý định dạng ngày dd/MM/yyyy
        ...
        java.util.Comparator<String> todayFirstComparator = (s1, s2) -> {
            try {
                ...
                // Cắt giờ về 00:00:00 để so sánh chuẩn với ngày hôm nay
                ...
                boolean d1IsPast = d1.before(today);
                boolean d2IsPast = d2.before(today);

                if (d1IsPast && !d2IsPast)
                    return 1;
                if (!d1IsPast && d2IsPast)
                    return -1;
                return d1.compareTo(d2); // Sắp xếp tương lai/hôm nay tăng dần
            } catch (Exception e) {
                return s1.compareTo(s2);
            }
        };

        rowSorter.setComparator(4, dateComparator); // Ngày bay ở cột số 4

        List<RowSorter.SortKey> sortKeys = new ArrayList<>();
        switch (type) {
            case 0 -> { // Mã vé mới nhất
                rowSorter.setComparator(0, java.util.Comparator.naturalOrder());
            ...
            case 2 -> sortKeys.add(new RowSorter.SortKey(4, SortOrder.ASCENDING)); // Ngày gần nhất
            case 3 -> sortKeys.add(new RowSorter.SortKey(4, SortOrder.DESCENDING)); // Ngày xa nhất
            case 4 -> { // Hôm nay & Tương lai lên đầu
                rowSorter.setComparator(4, todayFirstComparator);
                sortKeys.add(new RowSorter.SortKey(4, SortOrder.ASCENDING));
            }
            case 5 -> { // Giá tăng dần
                rowSorter.setComparator(6, java.util.Comparator.comparingDouble(o -> Double.parseDouble(o.toString())));
            ...
            case 7 -> { // Trạng thái A-Z
                rowSorter.setComparator(7, java.util.Comparator.naturalOrder());
    ...
    class PassengerRenderer extends JPanel implements TableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
                int row, int column) {
            setBackground(
                    isSelected ? table.getSelectionBackground() : (row % 2 == 0 ? AppColor.SURFACE : ZEBRA_COLOR));
            if (value != null) {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
            ...
            if (value != null) {
                ...
                if (text.equalsIgnoreCase("Phổ thông") || text.equalsIgnoreCase("Phổ thông đặc biệt")) {
                    badgeBgColor = new Color(224, 231, 255);
                    badgeFgColor = new Color(55, 65, 81);
                } else if (text.equalsIgnoreCase("Thương gia") || text.equalsIgnoreCase("Hạng nhất")) {
                    badgeBgColor = new Color(219, 234, 254);
                    badgeFgColor = new Color(29, 78, 216);
                } else if (text.equalsIgnoreCase("Đã xác nhận") || text.equalsIgnoreCase("Đã lên máy bay")) {
                    badgeBgColor = new Color(209, 250, 229);
                    badgeFgColor = new Color(6, 95, 70);
                } else if (text.equalsIgnoreCase("Chờ thanh toán")) {
                    badgeBgColor = new Color(254, 243, 199);
                    badgeFgColor = new Color(146, 64, 14);
                } else if (text.equalsIgnoreCase("Đã hủy")) {
                    badgeBgColor = new Color(254, 226, 226);
                    badgeFgColor = new Color(153, 27, 27);
                } else {
                    badgeBgColor = new Color(243, 244, 246);
                    badgeFgColor = new Color(75, 85, 99);
                }
            } else {
                text = "";
            }
