package bus.QuanLyVeBUS;

import java.text.DecimalFormat;
import java.util.List;

import dao.QuanLyVeDAO.VeDAO;
...
public class VeBUS {
        ...
        if (row != null) {
            ...
            if (seatClass != null) {
                if (seatClass.equalsIgnoreCase("Economy"))
                    ...
                    row[3] = "Thương gia";
                else if (seatClass.equalsIgnoreCase("Premium Economy"))
                    row[3] = "Phổ thông đặc biệt";
                else if (seatClass.equalsIgnoreCase("First Class"))
                    row[3] = "Hạng nhất";
            }
            if (row[4] instanceof Number)
                row[4] = df.format(((Number) row[4]).doubleValue());

            String status = (String) row[5];
            if (status != null) {
                if (status.equalsIgnoreCase("BOOKED"))
                    row[5] = "Chờ thanh toán";
                else if (status.equalsIgnoreCase("PAID"))
                    row[5] = "Đã xác nhận";
                else if (status.equalsIgnoreCase("CANCELLED"))
                    row[5] = "Đã hủy";
                else if (status.equalsIgnoreCase("CHECKED-IN"))
                    row[5] = "Đã lên máy bay";
            }
        }
        return row;
    }
    ...
    private void formatTicketRow(Object[] row, DecimalFormat df) {
        // Do bảng Danh sách đã thêm 2 cột, nên Hạng vé dời sang cột 5, Giá cột 6,
        // Status cột 7
        ...
        if (status != null) {
            if (status.equalsIgnoreCase("BOOKED"))
                row[7] = "Chờ thanh toán";
            else if (status.equalsIgnoreCase("PAID"))
                row[7] = "Đã xác nhận";
            else if (status.equalsIgnoreCase("CANCELLED"))
                row[7] = "Đã hủy";
            else if (status.equalsIgnoreCase("CHECKED-IN"))
                row[7] = "Đã lên máy bay";
        }
    }
