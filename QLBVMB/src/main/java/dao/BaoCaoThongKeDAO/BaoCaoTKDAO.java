...
public class BaoCaoTKDAO {
    public List<Object[]> getAllAirlines() {
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            throw new RuntimeException("Lỗi lấy danh sách hãng bay: " + e.getMessage());
        }
        return list;
    }
    ...
    public Object[] getDashboardSummary() {
        // Cột mới: Total_Revenue, Total_Bookings, Completed_Flights,
        ...
        try (Connection con = DBConnection.getConnection()) {
            if (con == null)
                throw new RuntimeException("Không thể kết nối đến Database (con = null)");
            try (Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sql)) {
                if (rs.next()) {
                    return new Object[] {
            ...
            throw new RuntimeException("Lỗi truy vấn VIEW_DASHBOARD_SUMMARY: " + e.getMessage());
        }
    ...
    public List<Object[]> getRevenueByClass() {
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            throw new RuntimeException("Lỗi truy vấn VIEW_REVENUE_BY_CLASS: " + e.getMessage());
        }
        return list;
    }
    ...
    public double[] getRevenueByMonth(int year) {
        ...
        // Bỏ lọc theo năm, lấy tất cả dữ liệu có trong View cộng dồn theo tháng để đảm
        // bảo luôn hiện số
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            throw new RuntimeException("Lỗi truy vấn VIEW_REVENUE_BY_MONTH: " + e.getMessage());
        }
        return result;
    }
    ...
    public double[] getDoanhThuTheoThang(int nam, String hangBay) {
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            try (PreparedStatement ps = con.prepareStatement(sql)) {
                ps.setInt(1, nam);
                ps.setString(2, hangBay);
                ps.setString(3, hangBay);
                ResultSet rs = ps.executeQuery();
                while (rs.next())
                    result[rs.getInt("thang") - 1] = rs.getDouble("tong");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // Object[]: {flightID, flightNumber, tuyen, departureTime, soVe, doanhThu,
    // tyLeLapDay, trangThai}
    ...
    // ──────────── Khoảng ngày nhỏ nhất & lớn nhất trong hệ thống ────────────
    // Trả về Date[2]: [0] = ngày sớm nhất, [1] = ngày muộn nhất
    public java.sql.Date[] getDateRange() {
        String sql = "SELECT " +
                "LEAST(NVL(MIN(b.BookingDate), SYSDATE), NVL(MIN(f.DepartureTime), SYSDATE)) AS minDate, " +
                "GREATEST(NVL(MAX(b.BookingDate), SYSDATE), NVL(MAX(f.DepartureTime), SYSDATE)) AS maxDate " +
                "FROM Booking b, Flight f";
        try (Connection con = DBConnection.getConnection()) {
            if (con == null) return new java.sql.Date[]{ new java.sql.Date(System.currentTimeMillis()), new java.sql.Date(System.currentTimeMillis()) };
            try (Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sql)) {
                if (rs.next()) {
                    java.sql.Date minDate = rs.getDate("minDate");
                    java.sql.Date maxDate = rs.getDate("maxDate");
                    if (minDate == null) minDate = new java.sql.Date(System.currentTimeMillis());
                    if (maxDate == null) maxDate = new java.sql.Date(System.currentTimeMillis());
                    return new java.sql.Date[]{ minDate, maxDate };
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
