...
public class AircraftDAO {
    ...
    public List<AircraftDTO> getAll() {
        ...
        String sql = "SELECT A.AircraftID, A.AirlineID, A.Model, A.Capacity, A.ManufactureYear, " +
                     "SUM(CASE WHEN S.Class = 'First Class' THEN 1 ELSE 0 END) AS FirstSeats, " +
                     "SUM(CASE WHEN S.Class = 'Business' THEN 1 ELSE 0 END) AS BusSeats, " +
                     "SUM(CASE WHEN S.Class = 'Premium Economy' THEN 1 ELSE 0 END) AS PremSeats, " +
                     "SUM(CASE WHEN S.Class = 'Economy' THEN 1 ELSE 0 END) AS EcoSeats " +
                     "FROM AIRCRAFT A " +
                     "LEFT JOIN SEAT S ON A.AircraftID = S.AircraftID " +
                     "GROUP BY A.AircraftID, A.AirlineID, A.Model, A.Capacity, A.ManufactureYear " +
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                dto.setFirstSeats(rs.getInt("FirstSeats"));
                dto.setBusSeats(rs.getInt("BusSeats"));
                dto.setPremSeats(rs.getInt("PremSeats"));
                dto.setEcoSeats(rs.getInt("EcoSeats"));
                list.add(dto);
            }
    ...
    public List<AircraftDTO> search(String keyword) {
        ...
        String sql = "SELECT A.AircraftID, A.AirlineID, A.Model, A.Capacity, A.ManufactureYear, " +
                     ...
                     "FROM AIRCRAFT A " +
                     "LEFT JOIN SEAT S ON A.AircraftID = S.AircraftID " +
                     "WHERE UPPER(A.AircraftID) LIKE ? OR UPPER(A.Model) LIKE ? OR UPPER(A.AirlineID) LIKE ? " +
                     "GROUP BY A.AircraftID, A.AirlineID, A.Model, A.Capacity, A.ManufactureYear " +
                     "ORDER BY A.AircraftID";
        try (Connection conn = DBConnection.getConnection();
                PreparedStatement ps = conn.prepareStatement(sql)) {
